...
/**
 * Initials of the folded words, for acronym matching: "Toggle bold text" → "tbt".
 * Digits stay as their own character so "h2" still matches "Heading 2".
 */
export function acronym(text: string): string {
	return foldedWords(text)
		.map((word) => word[0] ?? "")
		.join("");
}
